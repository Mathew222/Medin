...
try:
    ...
    logging.info("Using gemini-2.5-flash for analysis and chat.")
except Exception as e:
...
def translate_analysis(analysis_dict, target_language):
    ...
    try:
        # Create a translator instance for the requested target language
        translator_instance = GoogleTranslator(source='auto', target=target_language_code)
        ...
        def translate_value(value, dest_lang):
            if isinstance(value, str) and value.strip():
                try:
                    ...
                    # time.sleep(0.05)
                    translated = translator_instance.translate(value)
                    return translated
